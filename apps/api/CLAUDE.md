...
- **Phase 3.1B partial (LIVE — 2026-05-11)**: `archive_product` ONLY. Routes: `POST /api/v1/ai/products/:id/archive/request-approval` (initiator, `ai-agent:write-drafts`). Approve/reject/revoke/cancel/execute reuse the existing `ApprovalsAiController`. Same approval workflow plumbing as publish — HIGH risk, 2-min TTL, four-eyes, hashed token, `expectedUpdatedAt` stale-data, `payloadHash` binding, 3-attempt cap. Inverted state direction: validator `ArchiveValidationService.validateArchivable()` rejects unless the product is `isActive: true && deletedAt: null`; consume flips `isActive: false` (no `deletedAt` change — that's the recycle-bin path). Execute audit row uses `actionType: 'ARCHIVE'` (vs `PUBLISH` for publish). The `execute()` dispatch switches on `row.toolName` and computes `nextActiveState` accordingly. Phase 3.1B remainder (restore, update with pricing, order/rental status, inventory adjust, rental policy) NOT started — each wires through `ApprovalService` by adding a new `request*` method + a new `@Post(...)` initiator route + tool-specific consume dispatch.
...
- **products** - Full CRUD with variants, images, search, filtering, toggle active, soft delete/restore, admin listing (includes inactive), purchasePrice/minimumStock/supplier fields, `GET /products/by-id/:id` for edit page. `POST /products/bulk-import` accepts a multipart CSV file (max 5MB, 500 rows) for batch product creation — resolves category by slug or name, returns `{ created, failed, total, errors[] }` with per-row validation feedback.
  - **Public response whitelist** (2026-05-11): public endpoints `GET /products` and `GET /products/:slug` use explicit `select` clauses (`publicProductListSelect` / `publicProductDetailSelect` in `products.service.ts`) — NOT `include`. Storefront-required fields only; admin/internal columns are dropped at the DB level: `tenantId`, `purchasePrice`, `minimumStock`, `supplierName`, `supplierContact`, `lastRestockedAt`, `deletedAt`, `updatedAt`, plus variant `barcode` (POS-only). Admin paths (`findAllAdmin`, `findById`, `findDeleted`) keep `productIncludes` so the inventory/cost UI and the AI agent's `get_product` tool keep working. When the storefront needs a new product field, add it to BOTH the list and detail select — Prisma's typed return drops anything not selected, so omissions break the build immediately.
