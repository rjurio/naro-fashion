...
- `/dashboard/ai/assistant` - **AI Assistant chat** (added 2026-06-15). Natural-language wrapper around the `/api/v1/ai/*` read + draft tool surface. Calls `POST /ai-assistant/chat` with the conversation history; server runs the Anthropic tool-use loop (model: `claude-opus-4-8`) and returns `{ reply, toolCalls, iterations, inputTokens, outputTokens }`. Tool-call trace is shown as a collapsible `<details>` under each assistant message. Conversation history persists in `localStorage` (key `naro_ai_assistant_history`, capped at 50 turns) — clears via the header "Clear" button. When `ANTHROPIC_API_KEY` is not set on the API server, the `GET /ai-assistant/status` endpoint reports `configured: false` and the page shows a non-blocking banner.
- `/dashboard/ai/approvals` - **AI Approval Dashboard** (added 2026-05-13). Review pending/historical AI approval requests, approve/reject, copy the 64-hex token returned once per approval. Four-eyes enforced server-side (`forbidden_self_approval` → 403).
- `/dashboard/ai/role-assignments` - **AI Role Assignment** (added 2026-05-13). Assign `AI_AGENT_OPERATOR` / `AI_AGENT_APPROVER` to admin accounts. Server-side prevents an admin from holding both (four-eyes). Required entry-point after Phase 3.2 — SUPER_ADMIN no longer auto-holds `:approve`.
