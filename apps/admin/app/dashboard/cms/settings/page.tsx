...
const SETTING_DEFINITIONS: SettingDef[] = [
  ...
  { key: 'instagram_feed_layout', label: 'Instagram Feed Layout', type: 'select', group: 'Features', options: ['single_row', 'multi_row'], defaultValue: 'single_row', hint: 'Single row shows up to 6 posts (one desktop row). Multi row uses the row count below.' },
  { key: 'instagram_feed_rows', label: 'Instagram Feed Rows (multi-row only)', type: 'select', group: 'Features', options: ['2', '3', '4', '5'], defaultValue: '2', hint: 'How many rows when layout is multi-row. Each row fits 6 posts at desktop width (2×6 = 12 posts, 3×6 = 18, etc.). Ignored when layout is single-row.' },
  { key: 'instagram_feed_max_posts', label: 'Max Instagram Posts (hard cap)', type: 'select', group: 'Features', options: ['6', '12', '18', '24', '30'], defaultValue: '30', hint: 'Absolute upper limit. Layout + rows controls the visible count; this caps it (e.g. set to 12 to never show more than 12 posts even if rows × 6 is larger).' },
