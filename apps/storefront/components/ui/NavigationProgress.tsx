...
export default function NavigationProgress() {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      // Don't start if we're already on this exact path
  ...
  return (
    ...
    >
      <div
        className="h-full bg-gold-500 transition-all duration-200 ease-out"
        style={{ width: `${progress}%` }}
      />
      {/* Glow effect at the tip */}
      {progress < 100 && (
        <div
          className="absolute top-0 right-0 h-full w-24 -translate-x-0"
          style={{
            right: `${100 - progress}%`,
