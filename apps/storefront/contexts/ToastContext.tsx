...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToast = useCallback((variant: ToastVariant, message: string) => {
    ...
    setToasts(prev => [...prev, { id, variant, message }].slice(-5));
    setTimeout(() => {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div className="fixed top-4 right-4 z-[200] flex flex-col gap-2 pointer-events-none" role="region" aria-label="Notifications">
        {toasts.map(t => (
          <div
            key={t.id}
            className={`pointer-events-auto flex items-start gap-3 p-4 rounded-xl border shadow-lg max-w-sm w-full ${bg[t.variant]}`}
          >
            {icons[t.variant]}
